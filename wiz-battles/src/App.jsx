...
// --- CONNECTION SETUP ---
// If on Vercel (PROD), use Render URL. If on computer, use localhost.
const BACKEND_URL = import.meta.env.PROD 
  ? "https://wiz-battles-server.onrender.com" // <--- REPLACE THIS WITH YOUR ACTUAL RENDER URL LATER
  : "http://localhost:3001";
...
function WizBattles() {
  ...
  useEffect(() => {
    ...
    socket.on("waiting_for_opponent", () => {
        ...
        setGameState('waiting'); // UI state to show spinner/lock
    });

    socket.on("round_complete", (data) => {
        // Convert raw data back to Move Objects
        ...
        // Delay logic slightly for dramatic effect
        setTimeout(() => {
  ...
  const restartGame = () => {
    // For now, local restart. Real game needs socket emit "restart"
    setGameState('playing');
    setP1Energy(0);
    setP2Energy(0);
    setP1Move(null);
  ...
  return (
    <div className="h-screen max-h-screen w-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-950 via-slate-950 to-black text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      <div className="flex-1 w-full max-w-5xl flex flex-col relative justify-evenly py-2 px-4">
          ...
          {/* OPPONENT (Flip view based on role) */}
          <PlayerDisplay 
             role={myRole === 'p1' ? 'p2' : 'p1'} // Show enemy
            ...
            `}>
                {move && (
                    <div className="bg-slate-800 border border-slate-600 px-3 py-1 rounded-lg flex items-center gap-2">
                        {/* We hide enemy move name until resolution! */}
